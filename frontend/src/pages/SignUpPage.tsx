...
const SignUpPage = ()=>{
  ...
  const onFormLayoutChange = ({ size }: { size: SizeType }) => {
    ...
    const handleSignUp =(formValue: {email:string, password:string,name:string, surname:string,secondPassword: string})=>{
        const {email, password,name,surname,secondPassword} =formValue;
    ...
    <Layout style={{
        ...
        <div style={{fontSize:"24px"}}><b>Регистрация</b></div>
        <div className="logo-container" style={{alignItems:"center", display:"flex",justifyContent: "center",flexDirection:"column"}} >
            ...
            <div>Eсть аккаунт? <Link to={"/"}>Войти</Link></div>
    ...
    <FormItem
        ...
        >
          ...
          <Button style={{background:"#16223d",}} type="primary" htmlType="submit" size='large'  icon={<WindowsOutlined />}>Зарегистрироваться</Button>
        <Space />
      </Form>
      </Card>
    </Layout>);
